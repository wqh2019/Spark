...
export function createDevTools(ctx: ToolContext): Tool[] {
  ...
  const gitStash: Tool = {
    name: "git_stash",
    description: "Stash changes in the working directory (git stash push).",
    parameters: {
      message: {
        type: "string",
        description: "Optional stash message (default: auto-generated by git)",
      },
      include_untracked: {
        type: "boolean",
        description: "Include untracked files in the stash (git stash --include-untracked, default: false)",
      },
    },
    requiresConfirmation: true,
    async execute(args) {
      const gitArgs = ["stash", "push"];
      if (args.include_untracked === true) gitArgs.push("--include-untracked");
      if (args.message) gitArgs.push("-m", String(args.message));
      return runExec("git", gitArgs);
    },
  };

  const gitStashPop: Tool = {
    name: "git_stash_pop",
    description: "Apply and remove the latest stash (git stash pop). Use after git_stash to restore changes.",
    parameters: {},
    requiresConfirmation: true,
    async execute() {
      return runExec("git", ["stash", "pop"]);
    },
  };
